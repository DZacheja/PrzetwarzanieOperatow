...
namespace ControlManager {
    internal class ControlMoverOrResizer {
        ...
        internal static int CheckSize { get; set; }
        ...
        internal static bool ProportionalResize; //->Dodana wartość w celu proporcjonalnego rozszerzania
        ...
        internal static void Init(Control control, Control container) {
            ...
            double prop = (double)control.Width / (double)control.Height;
            control.MouseDown += (sender, e) => StartMovingOrResizing(control, e);
            control.MouseUp += (sender, e) => StopDragOrResizing(control);
            control.MouseMove += (sender, e) => MoveControl(container, e, prop);
        }

        internal static void DisableInit(Control control, Control container) {
            double prop = (double)control.Width / (double)control.Height;
            control.MouseDown -= (sender, e) => StartMovingOrResizing(control, e);
            control.MouseUp -= (sender, e) => StopDragOrResizing(control);
            control.MouseMove -= (sender, e) => MoveControl(container, e, prop);
        ...
        private static void UpdateMouseEdgeProperties(Control control, Point mouseLocationInControl) {
            ...
            MouseIsInLeftEdge = Math.Abs(mouseLocationInControl.X) <= CheckSize;
            MouseIsInRightEdge = Math.Abs(mouseLocationInControl.X - control.Width) <= CheckSize;
            MouseIsInTopEdge = Math.Abs(mouseLocationInControl.Y) <= CheckSize;
            MouseIsInBottomEdge = Math.Abs(mouseLocationInControl.Y - control.Height) <= CheckSize;
        }
        ...
        private static void UpdateMouseCursor(Control control) {
            ...
            if (MouseIsInLeftEdge && !ProportionalResize) {
                if (MouseIsInTopEdge) {
                ...
                if (MouseIsInTopEdge && !ProportionalResize) {
                    control.Cursor = Cursors.SizeNESW;
                } else if (MouseIsInBottomEdge) {
                    control.Cursor = Cursors.SizeNWSE;
                } else {
        ...
        private static void MoveControl(Control control, MouseEventArgs e, double prop) {
            ...
            if (_resizing) {
                if (MouseIsInLeftEdge && !ProportionalResize) {
                    ...
                    if (MouseIsInTopEdge && !ProportionalResize) {
                        ...
                        //Tylko tutaj proporcjonalnie
                    } else if (MouseIsInBottomEdge) {
                        control.Height = (e.Y - _cursorStartPoint.Y) + _currentControlStartSize.Height;
                        if (!ProportionalResize) {
                            control.Width = (e.X - _cursorStartPoint.X) + _currentControlStartSize.Width;
                        } else {
                            control.Width = (int)(control.Height * prop);
                        }
                    } else {
                        if (!ProportionalResize) {
                            control.Width = (e.X - _cursorStartPoint.X) + _currentControlStartSize.Width;
                ...
                } else if (MouseIsInTopEdge && !ProportionalResize) {
                    control.Height -= (e.Y - _cursorStartPoint.Y);
                    control.Top += (e.Y - _cursorStartPoint.Y);
                } else if (MouseIsInBottomEdge && !ProportionalResize) {
                    if (!ProportionalResize) {
                        control.Height = (e.Y - _cursorStartPoint.Y) + _currentControlStartSize.Height;
                    }
                } else {
